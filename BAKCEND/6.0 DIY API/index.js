...
app.get("/filter", (req,res) => {
  ...
  const filteredJokes = jokes.filter((joke) => joke.jokeType === typeToFilter);
...
app.post("/jokes", (req, res) => {
  ...
  if (!jokeText || !jokeType) {
    return res.status(400).send("Param missed");
  }
  else
  {
    jokes.push({
...
app.patch("/jokes/:id", (req, res) => {  
  ...
  const atualJoke = jokes.find((j) => j.id === id);

  const replacementJoke = {
    id: id,
    jokeText: req.body.text || atualJoke.jokeText,  //Si no se ingresa un valor lo deja tal cual estaba 
    jokeType: req.body.type || atualJoke.jokeType,
  }
